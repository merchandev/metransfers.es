...
add_action( 'wp_head', function() {
    ...
    if ( ( is_front_page() || is_home() ) && ! defined( 'WPSEO_VERSION' ) && ! function_exists( 'the_seo_framework' ) ) {
        echo '<meta name="description" content="' .
            esc_attr(
                'Reserva tu transfer privado desde o hacia el Aeropuerto de Barcelona, centro, hotel o puerto. ChÃ³fer profesional, precio cerrado y atenciÃ³n personalizada 24/7.'
            ) .
            '">' . "\n";
    }
...
// ==========================================
// YOAST SEO: OptimizaciÃ³n de CTR para Home y Hubs
// ==========================================
...
// =============================================================================
// FILTRO: localizar URLs del wp_nav_menu al idioma activo
// =============================================================================

/**
 * Rewrites internal nav menu hrefs to include the active language prefix,
 * so that clicking a menu item from /en/... stays in English.
 *
 * Skipped when:
 * - mt_lang() is 'es' (no prefix needed)
 * - The link points to an external domain
 * - The link already contains a language prefix
 * - The slug is 'rutas' (CPT archive â€” needs explicit router support first)
 */
add_filter( 'nav_menu_link_attributes', function( $atts, $menu_item, $args ) {
    ...
    // Strip existing language prefix to get the canonical slug.
    if ( ! empty( $segments ) && defined( 'MT_LANGS' ) && isset( MT_LANGS[ $segments[0] ] ) ) {
        array_shift( $segments );
    }

    $clean_path = implode( '/', $segments );

    // Skip CPT archives without explicit translated router support.
    $skip_slugs = array( 'rutas' );
    if ( ! empty( $segments ) && in_array( $segments[0], $skip_slugs, true ) ) {
        return $atts;
    }

    $localized = mt_localized_url( $clean_path );

    // Preserve fragment (#anchor).
...
// =============================================================================
// ROUTER: soporte para /LANG/rutas/ (archivo CPT traducido virtualmente)
// =============================================================================

add_filter( 'parse_request', function( $wp ) {

    if ( ! function_exists( 'mt_lang' ) || ! function_exists( 'mt_is_translated' ) ) {
        return $wp;
    }
    ...
    // Must be /LANG/rutas/ pattern.
    if ( count( $segments ) < 2 ) return $wp;
    if ( ! defined( 'MT_LANGS' ) || ! isset( MT_LANGS[ $segments[0] ] ) ) return $wp;
    if ( 'rutas' !== ( $segments[1] ?? '' ) ) return $wp;

    // Serve the Spanish rutas archive, translated.
...
// =============================================================================
// FILTRO: traducir tÃ­tulos del wp_nav_menu al idioma activo
// =============================================================================
