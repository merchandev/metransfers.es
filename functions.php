...
if ( ! defined( 'ME_TRANSFERS_VERSION' ) ) {
	define( 'ME_TRANSFERS_VERSION', '4.3.12' );
}
...
function me_transfers_scripts() {
	...
	$ajax_config = array(
		...
		'i18n'              => function_exists( 'mt_translate' ) ? array(
			'sending'   => mt_translate( 'Enviando...' ),
			'sent'      => mt_translate( 'Solicitud enviada correctamente.' ),
			'errorSend' => mt_translate( 'Error al enviar la solicitud.' ),
		) : array(),
	);
...
function me_transfers_hide_menus_checkhoteles() {
    ...
    $is_check_hoteles = in_array( 'check_hoteles', (array) $user->roles ) && ! in_array( 'administrator', (array) $user->roles );
    ...
    if ( $is_check_hoteles ) {
        ...
        $allowed_menus = array(
            ...
            'mt-hoteles-hub', // Menú propio de hoteles para CheckHoteles
            'edit.php?post_type=hotel_partner', // por si acaso
...
function me_transfers_restrict_checkhoteles_access() {
    ...
    if ( in_array( 'check_hoteles', (array) $user->roles ) && ! in_array( 'administrator', (array) $user->roles ) ) {
        ...
        if ( isset($_GET['page']) ) {
            ...
            $allowed_plugins = array('sg-cachepress', 'sg-security', 'agente-ia', 'wp-agente-ia', 'gyg-reviews', 'hotel-qr-reservations', 'mt-import-hotels', 'wptb-reservas');
...
add_filter( 'wpseo_opengraph_title', function( $title ) {
	if ( is_front_page() ) {
		return 'MeTransfers Barcelona - Traslados privados y tours exclusivos';
	}
	return $title;
}, 10 );
...
add_filter( 'wpseo_opengraph_desc', function( $desc ) {
	if ( is_front_page() ) {
		return 'Servicio de traslados privados, chófer por horas y tours exclusivos en Barcelona y Cataluña. Flota premium y puntualidad garantizada.';
	}
	return $desc;
}, 10 );
