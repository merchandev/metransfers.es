...
<div class="mt-hotel-phase-card mt-hotel-dashboard-table-card"><div class="mt-hotel-table-wrap"><table class="mt-hotel-table"><thead><tr><th>ID</th><th><?php esc_html_e( 'Fecha', 'me-transfers' ); ?></th><th><?php esc_html_e( 'Cliente', 'me-transfers' ); ?></th><th><?php esc_html_e( 'Trayecto', 'me-transfers' ); ?></th><th><?php esc_html_e( 'Vehículo', 'me-transfers' ); ?></th><th><?php esc_html_e( 'Distancia', 'me-transfers' ); ?></th><th><?php esc_html_e( 'Estado', 'me-transfers' ); ?></th><th><?php esc_html_e( 'Importe', 'me-transfers' ); ?></th></tr></thead><tbody>
<?php
foreach ( $data['rows'] ?? array() as $booking ) :
	?>
	<tr><td><a href="<?php echo esc_url( home_url( '/hoteles/reservas/' . absint( $booking->id ) . '/' ) ); ?>">#<?php echo esc_html( (string) $booking->id ); ?></a></td><td><?php echo esc_html( mysql2date( 'd/m/Y H:i', $booking->booking_date . ' ' . $booking->booking_time ) ); ?></td><td><?php echo esc_html( $booking->customer_name ); ?></td><td><?php echo esc_html( $booking->origin . ' → ' . $booking->destination ); ?></td><td><?php echo esc_html( \MeTransfers\HotelPortal\Services\HotelOperations::vehicleName( $booking ) ); ?></td><td><?php echo esc_html( number_format_i18n( (float) $booking->distance_km, 2 ) . ' km' ); ?></td><td><?php echo esc_html( \MeTransfers\HotelPortal\Services\HotelDashboard::statusLabel( $booking->status ) ); ?></td><td>
	<?php
