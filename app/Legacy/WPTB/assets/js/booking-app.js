    ...
    function initDetailsPage() {
        ...
        if (isDetailsPage) {
            ...
            if (bookingData.vehicle) {
                ...
                function updateLuggageLimits() {
                    const pax = parseInt($('#wptb-passengers').val(), 10) || 1;
                    const maxAllowed = pax * 2;
                    
                    $('#wptb-suitcases, #wptb-carryOns').attr('max', maxAllowed);
                    
                    if (parseInt($('#wptb-suitcases').val(), 10) > maxAllowed) {
                        $('#wptb-suitcases').val(maxAllowed);
                    }
                    if (parseInt($('#wptb-carryOns').val(), 10) > maxAllowed) {
                        $('#wptb-carryOns').val(maxAllowed);
                    }
                }

                $('#wptb-passengers').on('input', function () {
                    const pax = parseInt($(this).val(), 10) || 1;
                    if (pax > maxPax) {
                        $(this).val(maxPax);
                    }
                    updateLuggageLimits();
                ...
                // Initialize luggage limits on load
                updateLuggageLimits();

                $('#wptb-suitcases, #wptb-carryOns').off('input.luggageLimit').on('input.luggageLimit', function () {
                    const pax = parseInt($('#wptb-passengers').val(), 10) || 1;
                    const maxAllowed = pax * 2;
                    const currentCount = parseInt($(this).val(), 10) || 0;
